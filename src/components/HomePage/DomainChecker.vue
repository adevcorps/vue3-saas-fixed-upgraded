<template>
  <section>
    <div class="container mx-auto max-xl:max-w-[100%] max-xl:px-5">

      <div class="flex flex-wrap mt-28 max-md:mt-30 max-md:px-5">
        <div class="w-full flex items-center pl-9 mb-7 invisible max-md:visible">
            <img
              src="@/assets/images/home/domain-group/domain-checker.png"
              class="my-0"
            />
            <span class="text-purple font-bold">DOMAIN CHECKER</span>
        </div>
        <div class="lg:w-7/12 relative flex max-md:w-full">
          <img
            src="@/assets/images/home/domain-group/dash.png"
            class="w-11/12 h-11/12 z-10 max-md:w-full"
            alt=""
          />
          <img
            src="@/assets/images/home/domain-group/Polygon7.png"
            class="absolute right-10 max-md:w-full"
            alt=""
          />
          <img
            src="@/assets/images/home/domain-group/Polygon8.png"
            class="absolute right-12 max-md:w-full"
            alt=""
          />
        </div>
        <div class="lg:w-5/12 prose font-primary md:w-full  max-md:mb-5 max-md:mt-0">
          <div class="w-full flex items-center pl-9 mb-7 max-md:invisible">
            <img
              src="@/assets/images/home/domain-group/domain-checker.png"
              class="my-0"
            />
            <span class="text-purple font-bold">DOMAIN CHECKER</span>
          </div>
          <h1 class="text-[40px] leading-60 mb-5 max-md:text-[36px] max-md:leading-[1.2]">Instant Domain Checker</h1>
          <p class="w-9/12 text-xl m-0 mb-10 leading-9 max-md:w-full">
            Elevate your email strategy by validating domain MX records and identifying
            catch-all domains and spam traps before hitting send. Ensure your emails reach
            the right inboxes effortlessly.
          </p>
          <button class="w-50 py-3 px-6 bg-[#9B51E0] font-medium text-white rounded-full">
            Check Domain
          </button>
        </div>
      </div>
    </div>
  </section>
</template>
